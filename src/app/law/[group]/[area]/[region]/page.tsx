...
export default async function AreaRegion({ params, searchParams }: P) {
  ...
  const [all, { rows, count }, regions, st, rgc, logos] = await Promise.all([getPracticeAreas(), listByAreas([a.slug], { region: r.region_slug, page }), getRegions(), getState(r.state), getRegionGroupCounts(r.region_slug), getLogos(a.slug, r.region_slug, 20)]);
  ...
  return (<>
    ...
    <div className="wrap">
      <ListingList rows={rows} areas={areaMap(all)} name={`${a.name} lawyers in ${r.region_name}`} page={page} count={count} hrefFor={(p) => `/law/${g.slug}/${a.slug}/${r.region_slug}${p > 1 ? `?page=${p}` : ""}`} />
