  ...
  return (<>
    ...
    <div className="wrap py-14 grid gap-12 md:grid-cols-2 lg:grid-cols-3">
      {groups.map((g) => (<section key={g.slug}>
        <h2 className="text-2xl"><Link href={`/law/${g.slug}`} className="hover:underline">{g.name}</Link> <span className="text-sm text-muted font-body">{gc[g.slug] ?? 0}</span></h2>
        <ul className="mt-3 grid gap-1.5">{areas.filter((a) => a.group_slug === g.slug).map((a) => <li key={a.slug} className="flex justify-between gap-3 text-sm border-b border-line pb-1.5"><Link href={`/law/${g.slug}/${a.slug}`} className="hover:underline">{a.name}</Link><span className="text-muted tabular-nums">{counts[a.slug] ?? 0}</span></li>)}</ul>
      </section>))}
    </div></>);
}
