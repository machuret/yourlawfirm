...
import { legalImages } from "@/lib/imagery";
...
export default async function Law() {
  ...
  return (<>
    <Hero image={legalImages.library} credit="AI-generated editorial image" kicker="Find the right expertise" title="Areas of law" intro="Choose the area that fits your situation. Explore practical guides, documents to prepare, questions to ask and firms that work in each field." crumbs={[{ name: "Areas of law" }]} />
    <div className="wrap py-14 grid gap-12 md:grid-cols-2 lg:grid-cols-3">
      {groups.map((g) => (<section key={g.slug}>
        ...
        <p className="mt-3 text-sm leading-relaxed text-ink-2">{g.intro}</p>
