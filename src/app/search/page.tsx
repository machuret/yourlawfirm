...
export default async function Search({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  const sp = await searchParams; const q = (sp.q ?? "").trim(); const f = parseFilters(sp);
  const lat = sp.lat ? Number(sp.lat) : null, lng = sp.lng ? Number(sp.lng) : null;
  const [areas, rows, sugg] = await Promise.all([getPracticeAreas(), lat != null && lng != null ? searchNearby(lat, lng, sp.area || undefined) : q || sp.area ? searchText(q, sp.area || undefined, f) : Promise.resolve([]), q ? suggest(q) : Promise.resolve([])]);
  ...
  return (<>
    ...
    <div className="wrap py-10">
      ...
      {(q || sp.area || lat != null) ? <Results rows={rows} count={rows.length} areas={areaMap(areas)} name={`Search results for ${q}`} page={1} filters={f} hrefFor={() => ""} /> : null}
    </div></>);
}
