  ...
  return (<>
    ...
    <div className="wrap py-14 grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
      {ORDER.map((st) => (<section key={st} id={st} className="scroll-mt-24">
        <h2 className="text-2xl">{states.find((s) => s.state === st)?.name ?? st}</h2>
        <ul className="mt-3 grid gap-1.5">{regions.filter((r) => r.state === st).map((r) => <li key={r.region_slug} className="flex justify-between gap-3 text-sm border-b border-line pb-1.5"><Link href={`/locations/${r.region_slug}`} className="hover:underline">{r.region_name}</Link><span className="text-muted tabular-nums">{counts[r.region_slug] ?? 0}</span></li>)}</ul>
      </section>))}
    </div></>);
}
