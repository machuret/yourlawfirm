...
export default async function RegionPage({ params, searchParams }: P) {
  ...
  const [{ rows, count }, areas, groups, rgc, regions, st, logos] = await Promise.all([listByRegion(r.region_slug, page), getPracticeAreas(), getGroups(), getRegionGroupCounts(r.region_slug), getRegions(), getState(r.state), getLogos(null, r.region_slug, 24)]);
  ...
  return (<>
    ...
    <div className="wrap">
      ...
      <div className="mt-8"><ListingList rows={rows} areas={areaMap(areas)} name={`Lawyers in ${r.region_name}`} page={page} count={count} hrefFor={(p) => `/locations/${r.region_slug}${p > 1 ? `?page=${p}` : ""}`} /></div>
